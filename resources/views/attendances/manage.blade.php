...
@section('title', __('pages.attendance_manage.title'))
@section('subtitle', __('pages.attendance_manage.subtitle'))
...
@section('content')
    <div class="min-w-0">
            ...
            <div class="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <form method="GET" class="grid min-w-0 flex-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="flex min-w-0 flex-col gap-2 sm:col-span-2 sm:flex-row sm:items-end lg:col-span-1">
                        ...
                        @if($hasFilters)
                            <a href="{{ route('attendances.manage') }}" class="btn-secondary w-full shrink-0 sm:w-auto" title="Reset filter">Reset</a>
                        @endif
                    </div>
                </form>

                <a href="{{ route('attendances.create') }}" class="btn-primary flex w-full items-center justify-center lg:inline-flex lg:w-auto">
                    + {{ __('pages.attendance_manage.manual_input') }}
                </a>
            </div>
        ...
        <div class="space-y-2.5 lg:hidden">
            ...
            @empty
                <div class="panel p-8 text-center">
                    ...
                    @if($hasFilters)
                        <a href="{{ route('attendances.manage') }}" class="link-action mt-4 inline-block">Reset filter</a>
                    @endif
                </div>
            @endforelse
        </div>
        ...
        <div class="panel-table table-mobile-scroll attendance-table-shell hidden lg:block">
            <table class="table-readable table-readable--scroll-only attendance-history-table attendance-manage-table">
                <thead>
                    <tr>
                        ...
                        <th class="cell-manage-header">{{ __('pages.attendance_manage.update_status') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($attendances as $dayGroup)
                        <tr>
                            ...
                            <td class="cell-manage">
                                <div class="attendance-manage-list">
                                    @foreach($dayGroup->displayRecords() as $record)
                                        <div class="attendance-manage-item">
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-16 text-center">
                                <div class="mx-auto max-w-sm">
                                    ...
                                    @if($hasFilters)
                                        <a href="{{ route('attendances.manage') }}" class="mt-4 inline-block text-base font-semibold text-campfire-1 hover:underline">Reset filter</a>
                                    @endif
                                </div>
                            </td>
                        </tr>
