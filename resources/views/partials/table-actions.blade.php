@props([
    ...
    'deleteLabel' => 'Hapus',
    'accountCreate' => null,
    'accountCreateAction' => 'edit',
    'accountCreateLabel' => 'Tambah Akun',
    'accountCreateConfirm' => 'Buat akun login untuk pegawai ini?',
    'extras' => [],
    'layout' => 'inline',
])
...
<div {{ $attributes->merge(['class' => 'table-actions '.$layoutClass]) }} role="group" aria-label="Aksi baris">
    ...
    @if($accountCreate && $can($accountCreateAction))
        <form
            method="POST"
            action="{{ $accountCreate }}"
            class="table-actions__form"
            onsubmit="return confirm(@js($accountCreateConfirm))"
        >
            @csrf
            <button type="submit" class="table-action-btn table-action-btn--account" title="{{ $accountCreateLabel }}" aria-label="{{ $accountCreateLabel }}">
                <svg class="table-action-btn__icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M18 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM3 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 019.374 21c-2.331 0-4.512-.645-6.374-1.766z" />
                </svg>
                <span class="sr-only">{{ $accountCreateLabel }}</span>
            </button>
        </form>
    @endif
